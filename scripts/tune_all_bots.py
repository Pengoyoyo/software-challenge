#!/usr/bin/env python3
"""
Global eval-weight tuner for cython_v2 against all discovered bots.

Highlights:
- discovers all runnable python bot entrypoints via benchmark.py discovery
- optional preflight to filter broken opponents automatically
- GA optimization with checkpoint/resume
- uses side-switched matches and applies env weights only to target bot
- writes json checkpoint + markdown final analysis
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Tune cython_v2 eval weights against all discovered bots."
    )
...
def main() -> int:
    ...
    if args.log_dir is None:
        ...
        log_dir = ROOT / "log" / "tune_all_bots" / stamp
    else:
