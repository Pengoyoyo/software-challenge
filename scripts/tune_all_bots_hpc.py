#!/usr/bin/env python3
"""
High-throughput eval-weight tuner for cython_v2 against discovered bots.
...
DEFAULT_TARGET = ROOT / "bots" / "cython_v2" / "client_cython.py"
DEFAULT_TARGET_ENV = "CYTHON_V2_EVAL_PARAMS"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Tune cython_v2 eval weights with high-throughput parallel evaluation."
    )
    ...
    parser.add_argument(
        ...
        help="Comma-separated 5 floats (default: cython_v2 baseline).",
    )
    parser.add_argument(
        "--opponent",
        action="append",
...
def resolve_parallel_games(args: argparse.Namespace) -> tuple[int, int]:
    if args.parallel_games > 0:
        ...
        return args.parallel_games, cpu_cores
    ...
    auto = int(usable / args.cores_per_game)
    auto = max(1, auto)
...
def parse_weights(raw: str) -> tuple[float, float, float, float, float]:
    ...
    if len(parts) != 5:
        raise ValueError("exactly 5 weights are required")
...
def evaluate_weights(
    ...
    for opp in opponents:
        ...
        per_opponent[opp.path] = stats

    scores = [score_from_stats(s, error_penalty) for s in per_opponent.values()]
    total_w = sum(s.wins for s in per_opponent.values())
    total_l = sum(s.losses for s in per_opponent.values())
    total_d = sum(s.draws for s in per_opponent.values())
    total_e = sum(s.errors for s in per_opponent.values())
    total_g = sum(s.games for s in per_opponent.values())
...
def failed_eval_result(opponents: list[BotEntry], games_per_opponent: int) -> EvalResult:
...
def evaluate_pending_candidates_parallel(
    ...
    for idx, cand in enumerate(population, start=1):
        if cand.eval_result is None:
            pending.append((idx, cand))

    if not pending:
        return gid

    # Build per-game jobs across all pending candidates to keep workers busy
    # even if population is small.
    jobs: list[tuple[int, tuple[float, float, float, float, float], BotEntry, int, int]] = []
    for idx, cand in pending:
        for opp in opponents:
            for rep in range(args.games_per_opponent):
                jobs.append((idx, cand.weights, opp, rep, gid))
                gid += 1

    if not jobs:
        return gid

    max_workers = max(1, min(parallel_games, len(jobs)))
    print(
        f"[eval] pending={len(pending)} jobs={len(jobs)} "
        f"parallel_games={max_workers} games_per_candidate={games_per_eval}",
        flush=True,
    )

    per_candidate: dict[int, dict[str, OpponentStats]] = {}
    for idx, cand in pending:
        per_candidate[idx] = {opp.path: OpponentStats() for opp in opponents}

    def run_job(
        item: tuple[int, tuple[float, float, float, float, float], BotEntry, int, int]
    ) -> tuple[int, str, str, str]:
        idx, weights, opp, rep, game_id = item
        env_target = {args.target_env: format_weights(weights)}
        if rep % 2 == 0:
            bot_one, bot_two = target, opp
            env_one, env_two = env_target, None
            target_side = "ONE"
        ...
        game = run_game(
            ...
            run_dir=run_dir,
            keep_logs=args.keep_game_logs,
            env_one=env_one,
            env_two=env_two,
        )
        return idx, opp.path, str(game.get("winner")), target_side

    future_map: dict[concurrent.futures.Future[tuple[int, str, str, str]], tuple[int, str]] = {}
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as pool:
        for job in jobs:
            idx, _weights, opp, _rep, _game_id = job
            fut = pool.submit(run_job, job)
            future_map[fut] = (idx, opp.path)

        for fut in concurrent.futures.as_completed(future_map):
            idx, opp_path = future_map[fut]
            stats = per_candidate[idx][opp_path]
            stats.games += 1

            try:
                _idx, _opp_path, winner, target_side = fut.result()
                if winner == target_side:
                    stats.wins += 1
                elif winner in ("ONE", "TWO"):
                    stats.losses += 1
                ...
                stats.errors += 1
                print(f"  [{idx:>2}/{len(population)}] job error: {exc}", flush=True)

    for idx, cand in pending:
        per_opp = per_candidate[idx]
        scores = [score_from_stats(s, args.error_penalty) for s in per_opp.values()]
        total_w = sum(s.wins for s in per_opp.values())
        total_l = sum(s.losses for s in per_opp.values())
        total_d = sum(s.draws for s in per_opp.values())
        total_e = sum(s.errors for s in per_opp.values())
        total_g = sum(s.games for s in per_opp.values())
        ev = EvalResult(
            fitness=(sum(scores) / max(1, len(scores))),
            worst_fitness=(min(scores) if scores else -1.0),
            wins=total_w,
...
def build_next_population(
    ...
    for _ in range(immigrants):
        ...
        next_pop.append(Candidate(weights=tuple(rand_w)))  # type: ignore[arg-type]
...
def preflight_opponents(
    target: BotEntry,
    opponents: list[BotEntry],
    initial_weights: tuple[float, float, float, float, float],
    args: argparse.Namespace,
    run_dir: Path,
    start_game_id: int,
    parallel_games: int,
    ...
    tasks: list[tuple[int, BotEntry, int]] = []
    for idx, opp in enumerate(opponents, start=1):
        tasks.append((idx, opp, gid))
        gid += args.preflight_games

    results: dict[int, tuple[BotEntry, EvalResult]] = {}
    max_workers = max(1, min(parallel_games, len(tasks)))
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as pool:
        future_map: dict[concurrent.futures.Future[tuple[EvalResult, int]], tuple[int, BotEntry]] = {}
        for idx, opp, opp_gid in tasks:
            fut = pool.submit(
                evaluate_weights,
                ...
                error_penalty=args.error_penalty,
                next_game_id=opp_gid,
            )
            future_map[fut] = (idx, opp)

        for fut in concurrent.futures.as_completed(future_map):
            idx, opp = future_map[fut]
            try:
                res, _ = fut.result()
            except Exception as exc:
                res = failed_eval_result(opponents=[opp], games_per_opponent=args.preflight_games)
                print(f"[preflight] {idx:>2}/{len(opponents)} ERROR {safe_rel(Path(opp.path))}: {exc}", flush=True)
            results[idx] = (opp, res)

    for idx in range(1, len(opponents) + 1):
        if idx not in results:
            continue
        opp, res = results[idx]
        valid_games = res.wins + res.losses + res.draws
        ok = valid_games > 0
...
def write_final_analysis(
    ...
    lines: list[str] = []
    lines.append("# Global Tuning Analysis")
    lines.append("")
...
def main() -> int:
    ...
    if args.log_dir is None:
        ...
        log_dir = ROOT / "log" / "tune_all_bots_hpc" / stamp
    else:
    ...
    for gen in range(start_gen, args.generations):
        ...
        # optional noise reduction for top candidates
