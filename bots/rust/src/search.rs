...
impl TimeManager {
    ...
    fn tick(&mut self) {
        ...
        self.check_mask = if rem_ns < 30_000_000 {
            3   // < 30ms: check every 4 nodes
        } else if rem_ns < 50_000_000 {
            7   // < 50ms: check every 8 nodes
        } else if rem_ns < 80_000_000 {
            15  // < 80ms: check every 16 nodes
...
impl SearchEngine {
    ...
    fn quiescence(
        ...
        // Connectivity-improving quiet moves (only in shallow qdepth and when time allows)
        // Skip this expensive section if time is critical (< 50ms remaining)
        if qdepth < 2 && timer.remaining_ns() > 50_000_000 {
            ...
            for mv in all_moves.as_slice() {
                // Time check inside this expensive loop
                timer.tick();
                if timer.timed_out() {
                    break;
                }
