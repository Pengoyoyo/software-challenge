use crate::bitboard::{get_neighbor_masks, pop_lsb, Bitboard};
...
impl Position {
    ...
    pub fn unmake_move(&mut self, undo: &Undo) {
        ...
        // Rebuild component data from the now-restored bitboards (O(8) BFS, faster than
        // copying 471 bytes back from Undo).
        let moved_owner = piece_owner(moved);
        self._rebuild_components(moved_owner);
        if captured_owner != 0 {
            self._rebuild_components(captured_owner);
